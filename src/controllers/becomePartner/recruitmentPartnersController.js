...
const RecruitmentPartners = require('../../model/becomePartner/RecruitmentPartners');
...
const getRecruitmentPartners = async (req, res) => {
    try {
        const records = await RecruitmentPartners.find().sort({ createdAt: -1 }).lean();
...
const getActiveRecruitmentPartner = async (req, res) => {
    try {
        const record = await RecruitmentPartners.findOne({ isActive: true }).sort({ createdAt: -1 }).lean();
        if (!record) {
...
const createRecruitmentPartner = async (req, res) => {
    try {
        ...
        const created = await RecruitmentPartners.create({
            title: title.trim(),
...
const updateRecruitmentPartner = async (req, res) => {
    try {
        ...
        const updated = await RecruitmentPartners.findByIdAndUpdate(id, updateData, {
            new: true,
            runValidators: true,
        });
...
const deleteRecruitmentPartner = async (req, res) => {
    try {
        ...
        const deleted = await RecruitmentPartners.findByIdAndDelete(id);
